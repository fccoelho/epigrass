...
def get_sims(fname):
    ...
    if os.path.exists(full_path):
        ...
        with engine.connect() as con:
            ...
            sim_names = [
                s[0] for s in sims if not (s[0].endswith("_meta") or s[0].endswith("e"))
            ]
        return sim_names
    else:
...
def create_simulation_table(model_path, simulation_run):
    ...
    epi_vars = [
        c for c in df.columns if c not in ["name", "time", "geocode", "lat", "longit"]
    ]
    ...
    existing_mapping = {
        k: v for k, v in column_mapping.items() if k in table_data.columns
    }
    ...
    fig.add_trace(
        go.Choropleth(
            ...
            customdata=np.column_stack(
                (mapdf["name"], mapdf["prevalence"], mapdf["arrivals"])
            ),
...
def create_final_map(model_path, map_selector, simulation_run):
    ...
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            marker=dict(
                color=map10["totalcases"], colorscale="YlOrRd", showscale=False
            ),
    ...
    if hasattr(mapdf, "geometry") and not mapdf.empty:
        ...
        fig.add_trace(
            go.Choropleth(
                ...
                colorbar=dict(title="Casos Totais", x=1.02, len=0.8),
                showlegend=False,
            ),
            row=1,
            col=2,
        ...
        if len(mapdf) > 0:
            ...
            fig.update_geos(
                ...
                lataxis_range=[
                    mapdf["lat"].min() - lat_padding,
                    mapdf["lat"].max() + lat_padding,
                ],
                lonaxis_range=[
                    mapdf["lon"].min() - lon_padding,
                    mapdf["lon"].max() + lon_padding,
                ],
                projection_scale=1,
            )

    # Update layout
    fig.update_layout(title="Estado Final da Epidemia", height=600, showlegend=False)
...
def create_network_viz(model_path, localities):
    ...
    if G.order() == 0:
        return go.Figure().add_annotation(
            text="Nenhum arquivo de rede encontrado",
            xref="paper",
            yref="paper",
            x=0.5,
            y=0.5,
    ...
    if not nodeloc:
        return go.Figure().add_annotation(
            text="Localidade não encontrada na rede",
            xref="paper",
            yref="paper",
            x=0.5,
            y=0.5,
    ...
    if mapdf.empty:
        return go.Figure().add_annotation(
            text="Arquivo de mapa não encontrado",
            xref="paper",
            yref="paper",
            x=0.5,
            y=0.5,
    ...
    if hasattr(mapdf.geometry, "centroid"):
        ...
        return go.Figure().add_annotation(
            text="Coordenadas não disponíveis no mapa",
            xref="paper",
            yref="paper",
            x=0.5,
            y=0.5,
...
def create_temporal_map(model_path, simulation_run, time_slider):
    ...
    # Merge with map data
    name_col = (
        [c for c in mapdf.columns if df_time.name.iloc[0] in list(mapdf[c])][0]
        if len(df_time) > 0
        else "name"
    )
    mapa_t = pd.merge(
        mapdf,
        df_time[["name", "time"] + variables],
        left_on=name_col,
        right_on="name",
        how="left",
    )
    ...
    main_var = (
        "Infectious"
        if "Infectious" in variables
        else (variables[0] if variables else "incidence")
    )
    ...
    fig.add_trace(
        go.Choropleth(
            ...
            colorbar=dict(title=main_var.capitalize(), x=1.02),
            showlegend=False,
        )
    )
    ...
    if len(mapa_t) > 0 and "lat" in mapa_t.columns and "lon" in mapa_t.columns:
        ...
        fig.update_geos(
            ...
            center=dict(lat=lat_center, lon=lon_center),
            lataxis_range=[
                mapa_t["lat"].min() - lat_padding,
                mapa_t["lat"].max() + lat_padding,
            ],
            lonaxis_range=[
                mapa_t["lon"].min() - lon_padding,
                mapa_t["lon"].max() + lon_padding,
            ],
            projection_scale=1,
        )
...
def create_time_series(model_path, simulation_run, localities):
    ...
    fig = make_subplots(
        rows=n_rows, cols=n_cols, subplot_titles=variables, vertical_spacing=0.1
    )
    ...
    fig.update_layout(
        title=f"Séries Temporais - {localities}", height=400 * n_rows, showlegend=False
    )
...
def create_dashboard(pth: str):
    ...
    with gr.Blocks() as demo:
        gr.HTML(
            """<img src="https://github.com/fccoelho/epigrass/blob/master/Epigrass/egicon.png?raw=true"> <h1 align="center">Epigrass Dashboard</h1>"""
        )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                map_selector = gr.Dropdown(
                    label="📁 Arquivo de Mapa", choices=[], value=None
                )

                simulation_run = gr.Dropdown(
                    label="🎯 Simulação", choices=[], value=None
                )

                localities = gr.Dropdown(label="📍 Localidades", choices=[], value=None)

                time_slider = gr.Slider(
                    minimum=1, maximum=300, value=1, step=1, label="⏰ Tempo"
                )
        ...
        simulation_run.change(
            fn=get_meta_info, inputs=[model_path, simulation_run], outputs=[meta_info]
        )
        ...
        localities.change(
            fn=zoom_to_location, inputs=[model_path, localities], outputs=[final_map]
        )
